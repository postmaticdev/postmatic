...
export const RoleKnowledgeSchema = z.object({
  ...
  hashtags: z.array(z.string()).min(1, "Hashtags is required").refine(
    (values) => values.every((value) => value.startsWith("#")),
  ...
  platforms: z.array(z.string()).min(1, "Platforms is required"),
});
